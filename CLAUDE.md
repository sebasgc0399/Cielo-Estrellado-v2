...
- The legacy sky now has direct ownership assigned (`ownerUserId` set, `owner` membership created). The claim legacy flow is superseded as an active product front.
- The runtime still carries claim-related types in domain contracts (to be cleaned up in a subsequent simplification phase). The migration tooling remains available for operational validation (to be reviewed in a separate archival phase).
